...
feature_path="./data/"
Cor_descriptor_data = "Opt_RFE_descriptor.csv"
data = pd.read_csv(feature_path + Cor_descriptor_data)
data = data.drop(columns=['ID'])  
  
X = data.drop(columns=['diffusion'])
y = data['diffusion'] 
  
X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.1, random_state=42)  

select_feature=['ABC', 'LogEE_A', 'VR1_A', 'VR2_A', 'VR3_A', 'nN', 'nS']
...
n_estimator = int(10)
max_depths = int(2)
min_samples_split=int(np.round(9.15))
min_samples_leaf=int(np.round(9.10))
...
plt.tight_layout()
plt.savefig('./figure/shap_importance.png')
plt.close()
